...
def knapsack_randomized_local_search(items, knapsack_capacity, solution, weight, value, cnt_max=30):
    best_s = solution
    best_val = value
    best_weight = weight
    cnt = 0
    ...
    while cnt < cnt_max:
        # Erzeugung einer Zufallslösung
        s_cur, cur_weight, cur_value = generate_random_solution(items, knapsack_capacity)
        # Aufruf lokale Suche
        solution, weight, value = knapsack_local_search(items, knapsack_capacity, s, cur_weight, cur_value)
        # Wenn neue Lösung besser als alte, dann speichern
        if cur_value > best_val:
            best_val = cur_value
            best_s = s_cur

    return best_s, best_weight, best_val 
...
def main():
    ...
    for file in file_paths:
        ...
        print(f"Total Weight: {round(weight, 2)}")
        print(f"Total Value: {round(value, 2)}")
        # Lösung verbessern mit Randomized Locas Search
        solution = [0] * len(items)
        solution, weight, value = knapsack_randomized_local_search(items, knapsack_capacity, solution, 0, 0)
        index_list = [i + 1 for i in range(len(items)) if solution[i] == 1]
        print(f"Solution: {', '.join(map(str, index_list))}")
        print(f"Total Weight: {round(weight, 2)}")
        print(f"Total Value: {round(value, 2)}")
